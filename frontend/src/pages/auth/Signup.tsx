...
function checkPw(pw: string): PwState {
  return {
    length: pw.length >= 8,
    upper: /[A-Z]/.test(pw),
    lower: /[a-z]/.test(pw),
...
export default function Signup() {
  ...
  const pw = checkPw(password);
  const pwScore = Object.values(pw).filter(Boolean).length;
  const pwColors = [
    "bg-red-400",
    "bg-red-400",
    "bg-amber-400",
    ...
    <div className="space-y-6">
      <div>
        ...
        <p className="text-muted-foreground text-sm mt-2">
          Start managing your spice business in minutes
        </p>
      </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-5">
            ...
            <div className="space-y-2 pt-1">
              <div className="flex gap-1.5">
                {[0, 1, 2, 3].map((i) => (
                  <div
                    key={i}
                    className={`h-1.5 flex-1 rounded-full transition-colors duration-300 ${i < pwScore ? pwColors[pwScore] : "bg-muted"}`}
                  />
                ))}
              </div>
                ...
                {[
                  ...
                  <div key={req.label} className="flex items-center gap-1.5">
                    <IconCircleCheck
                      size={13}
                      className={
                        req.ok ? "text-emerald-500" : "text-muted-foreground/40"
                      }
                    />
                    <span
                      className={`text-xs ${req.ok ? "text-foreground" : "text-muted-foreground"}`}
                    >
                      {req.label}
                    </span>
                  </div>
        ...
        >
          {loading ? (
            ...
            <>
              Create account <IconArrowRight size={17} />
            </>
          )}
        </Button>
      </form>

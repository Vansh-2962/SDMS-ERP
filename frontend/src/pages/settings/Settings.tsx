...
import Bank from "@/features/profile/component/Bank";
import SettingsComp from "@/features/profile/component/Settings";
...
const automationSettings = [
  {
    key: "wa_order_confirm",
    label: "WhatsApp Order Confirmation",
    desc: "Send order confirmation to customer on WhatsApp",
    enabled: true,
  },
  {
    key: "wa_invoice",
    label: "WhatsApp Invoice",
    desc: "Auto-send invoice on WhatsApp after billing",
    enabled: true,
  },
  {
    key: "payment_reminder",
    label: "Payment Reminders",
    desc: "Auto-remind customers before due date",
    enabled: true,
  },
  {
    key: "birthday_wishes",
    label: "Birthday & Anniversary Wishes",
    desc: "Send greetings to retailer owners",
    enabled: false,
  },
  {
    key: "sms_alerts",
    label: "SMS Alerts",
    desc: "Send SMS for critical updates",
    enabled: false,
  },
  {
    key: "auto_backup",
    label: "Auto Backup",
    desc: "Daily automated data backup",
    enabled: true,
  },
  {
    key: "low_stock_alert",
    label: "Low Stock Alert",
    desc: "Notify when product stock falls below reorder level",
    enabled: true,
  },
  {
    key: "otp_login",
    label: "OTP Login",
    desc: "Require OTP for salesman login",
    enabled: false,
  },
];
            ...
            <Card className="xl:col-span-2 border border-border shadow-sm">
              ...
              <CardContent>
                <div className="grid grid-cols-2 sm:grid-cols-3 gap-2">
                  {allModules.map((mod) => {
                    const hasPermission =
                      selectedRole.permissions.includes(mod);
                    return (
                      <div
                        key={mod}
                ...
                <p className="text-xs text-muted-foreground">
                  ...
                  role. Click a module to toggle access.
                </p>
              </CardContent>
            </Card>
          </div>
                ...
                <thead>
                  <tr className="bg-muted/50 border-b border-border">
                    {[
                      ...
                      <th
                        key={h}
                        className="text-left text-xs font-semibold py-3 px-4 whitespace-nowrap"
                      >
                        {h}
                      </th>
                    ))}
                  ...
                  {users.map((u) => {
                    ...
                    return (
                      ...
                      >
                        ...
                        <td className="py-3 px-4">
                          <span
                            className={`px-2 py-0.5 rounded-full text-xs font-medium ${roleObj?.color || "bg-gray-100 text-gray-700"}`}
                          >
                            {u.role}
                          </span>
                        </td>
                        ...
                        <td className="py-3 px-4">
                          <span
                            className={`px-2 py-0.5 rounded-full text-xs font-medium ${u.status === "Active" ? "bg-emerald-100 text-emerald-700" : "bg-red-100 text-red-700"}`}
                          >
                            {u.status}
                          </span>
                        </td>
        ...
        <TabsContent value="automation" className="mt-4">
          <div className="grid grid-cols-1 xl:grid-cols-2 gap-4">
            <Card className="border border-border shadow-sm">
              <CardHeader className="pb-2">
                <CardTitle className="font-heading text-sm font-semibold flex items-center gap-2">
                  <IconBell size={16} className="text-primary" /> Automation
                  Settings
                </CardTitle>
              </CardHeader>
            ...
            <div className="space-y-4">
              <Card className="border border-border shadow-sm">
                <CardHeader className="pb-2">
                  <CardTitle className="font-heading text-sm font-semibold flex items-center gap-2">
                    <IconBrandWhatsapp size={16} className="text-emerald-500" />{" "}
              ...
              <Card className="border border-border shadow-sm">
                ...
                <CardContent className="space-y-3">
                  ...
                  <div className="space-y-1.5">
                    ...
                    <Input
                      placeholder="billing@goldspice.com"
                      className="h-8 text-sm"
                    />
                  </div>
                  <Button
              ...
              <Card className="border border-border shadow-sm">
                <CardHeader className="pb-2">
                  <CardTitle className="font-heading text-sm font-semibold flex items-center gap-2">
                    <IconDeviceMobile size={16} className="text-violet-500" />{" "}
                    SMS Configuration
                  </CardTitle>
                </CardHeader>
                <CardContent className="space-y-3">
